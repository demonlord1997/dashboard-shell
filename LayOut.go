...
func layout(g *gocui.Gui) error {
    ...
    if iShowV3 {
        beginBoxPosX = maxX/2 - 3*g_folderLength/2 + 3
        endBoxPosX = maxX-beginBoxPosX-1
    } else {
        beginBoxPosX = (maxX-72)/2 - 1
        endBoxPosX = (maxX+75)/2-1
    }
    if iShowV3 {
    ...
    if v, err := g.SetView("v2", beginBoxPosX+g_folderLength+1, beginBoxPosY, endBoxPosX, beginBoxPosY+g_itermHeight); err != nil {
        if err != gocui.ErrUnknownView {
            return err
        }
        g_varray = append(g_varray, v)
